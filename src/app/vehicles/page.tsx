...
export default function VehiclesPage() {
  ...
  useEffect(() => {
    const fetchVehicles = async () => {
      try {
        ...
        if (!token)
          throw new Error("Kein Auth-Token vorhanden. Bitte einloggen.");
  ...
  return (
    <div className="mx-auto max-w-4xl p-4">
      <h1 className="mb-4 text-2xl font-bold">Meine Fahrzeuge</h1>
      <div className="mb-4 space-y-2">
        <Label>Typ</Label>
        <Input
          value={newVehicle.type || ""}
          onChange={(e) =>
            setNewVehicle((prev) => ({ ...prev, type: e.target.value }))
          }
        />
        <Label>Marke</Label>
        <Input
      ...
      <h2 className="mb-2 text-xl font-semibold">Meine Fahrzeuge:</h2>
