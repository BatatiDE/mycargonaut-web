...
export default function EditVehiclePage() {
  ...
  return (
    <div className="mx-auto max-w-2xl p-6">
      <h1 className="mb-4 text-2xl font-bold">Fahrzeug bearbeiten</h1>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div>
          <label className="block font-medium">Typ</label>
          <input {...register("type")} className="w-full rounded border p-2" />
          {errors.type && <p className="text-red-500">{errors.type.message}</p>}
        </div>
        <div>
          <label className="block font-medium">Marke</label>
          <input {...register("brand")} className="w-full rounded border p-2" />
          {errors.brand && (
        ...
        <div>
          ...
          <input {...register("model")} className="w-full rounded border p-2" />
          {errors.model && (
