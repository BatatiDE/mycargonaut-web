...
interface OfferRequestFormProps {
  ...
  setFromLocation: (location: { lat: number; lng: number } | null) => void;
  setToLocation: (location: { lat: number; lng: number } | null) => void;
}
  ...
  useEffect(() => {
    async function updateAddress() {
      if (fromLocation) {
        ...
        setForm((prev) => ({ ...prev, startingPoint: address }));
      }
      if (toLocation) {
        const address = await getAddressSuggestions(
          toLocation.lat,
        ...
        setForm((prev) => ({ ...prev, destinationPoint: address }));
      }
    }
