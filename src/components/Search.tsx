...
import { SearchProps, SearchCriteria } from "@/types/searchProps"
...
export default function Search({ action }: SearchProps) {
    ...
    const handleSearch = (e: React.FormEvent) => {
        ...
        const filteredRides = trips.filter(trip =>
            (!startingPoint || trip.startingPoint.toLowerCase().includes(startingPoint.toLowerCase())) &&
            (!destinationPoint || trip.destinationPoint.toLowerCase().includes(destinationPoint.toLowerCase())) &&
    ...
    return (
        <form onSubmit={handleSearch} className="space-y-4">
            <div className="space-y-2">
                ...
                <Input id="from" value={startingPoint} onChange={(e) => setStartingPoint(e.target.value)} />
            </div>
            <div className="space-y-2">
                <Label htmlFor="to">Nach</Label>
                <Input id="to" value={destinationPoint} onChange={(e) => setDestinationPoint(e.target.value)} />
            </div>
            <div className="space-y-2">
            ...
            <div className="flex space-x-4">
                <Button type="submit" className="flex-1">Suchen</Button>
                <Button type="button" variant="outline" onClick={handleReset} className="flex-1">Zurücksetzen</Button>
            </div>
        </form>
    )
}
